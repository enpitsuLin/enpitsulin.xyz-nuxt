...
<template>
  <article class="md:grid md:grid-cols-4 md:items-baseline">
    <div class="group relative flex flex-col items-start md:col-span-3">
      ...
      <time class="relative z-10 order-first mb-3 flex items-center pl-3.5 text-sm text-zinc-500 md:hidden dark:text-zinc-500" :datetime="article.publishAt">
    ...
    <div relative z-10 order-first class="hidden md:flex" flex="col items-start">
      <time class="mb-3 mt-1 text-sm text-zinc-500 dark:text-zinc-500" :datetime="article.publishAt">
        {{ formatDate(new Date(article.publishAt), 'MMM DD YYYY', { locales: 'zh-Hans' }) }}
      </time>
